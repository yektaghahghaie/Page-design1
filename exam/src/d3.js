...
svg.selectAll(".bar")
  ...
  const marginBottom = 20;
  const marginLeft = 40;
  const width = 640 - marginLeft - marginRight; // Adjusted for margins
  const height = 400 - marginTop - marginBottom; // Adjusted for margins
  
  var svg = d3.select("body").append("svg")
      .attr("width", width + marginLeft + marginRight)
      .attr("height", height + marginTop + marginBottom)
      .append("g")
      .attr("transform", `translate(${marginLeft},${marginTop})`); // Adjust the group within the SVG
  
  var x = d3.scaleBand()
      .domain(json.data.heatmap.map(function (d) { return d.end_time; }))
      .range([0, width]) // Adjusted range for x-axis
      .padding(0.1);
  
  var y = d3.scaleLinear()
      .domain([0, d3.max(json.data.heatmap, function (d) { return d.value; })])
      .range([height, 0]); // Corrected range for y-axis
  
  svg.append("g")
      .attr("transform", `translate(0,${height})`) // Corrected transform
      .call(d3.axisBottom(x));
